"""
naive_stopwords_stemmed.py: Runs a Multinomial Naive Bayes Classifier to predict a user selected personality trait.
The data essays are preprocessed by removing stop words and stemming the words.

Example usage:  python naive_stopwords_stemmed.py <trait> <custom_alpha>
...
def classify(trait_arg, alpha):
    ...
    x = df['essay'][1:]
    x = x.str.lower()
    x = x.apply(lambda k: ' '.join([stemmer.stem(word) for word in k.split() if word not in
